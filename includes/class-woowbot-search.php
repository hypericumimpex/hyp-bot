...
if ( ! class_exists( 'WoowBot_Search' ) ) :
    ...
    class WoowBot_Search {
		...
		public function query_phrase($keyword){
			global $wpdb;
			
			$sql = "SELECT ID FROM ". $wpdb->prefix."posts where post_type in ('product') and post_status='publish' and ((post_title REGEXP '[[:<:]]".$keyword."[[:>:]]') or (post_content REGEXP '[[:<:]]".$keyword."[[:>:]]')) order by ID DESC";
			$limit = " Limit 0, 50";
        ...
        public function search( $keyword = ''  ) {
			...
			//phrase matching
			if(count(array_unique( explode( ' ', $s ) ))>1){
				$posts_ids = $this->query_phrase($s);
				if(empty($posts_ids)){
