<?php
		...
		wp_register_style('qlcd-wow-chatbot-admin-style-email', QCLD_wpCHATBOT_PLUGIN_URL.'/css/email_subscription.css', '', QCLD_wpCHATBOT_VERSION, 'screen');
		wp_enqueue_style('qlcd-wow-chatbot-admin-style-email');

		wp_register_script('qcld-wow-chatbot-email-subscription-js', QCLD_wpCHATBOT_PLUGIN_URL.'/js/email_subscription.js', array('jquery'), true);
            wp_enqueue_script('qcld-wow-chatbot-email-subscription-js');
		...
		$mainurl = admin_url( 'admin.php?page=wow-email-subscription');
		
	?>	
		<div class="qchero_sliders_list_wrapper">
			<div class="sld_menu_title">
				<h2><?php echo esc_html__('Email Subscription List', 'qc-opd') ?></h2>

			</div>
			
			<div class="sld_menu_title sld_menu_title_align"><?php echo ($customPagHTML); ?><span><a class="button-primary" href="<?php echo admin_url( 'admin-post.php?action=woobprint.csv' ); ?>">Export All Contacts</a> Total <?php echo esc_html($total); ?></span> </div>
			...
			<br>
			<button class="button-primary" id="wpbot_submit_email_form">Delete All</button>
